...
int main(){
	...
	{
		printf("Error al crear socket : %d",WSAGetLastError());
		return 1;
	}
	...
	if((network_socket = socket(AF_INET , SOCK_STREAM , IPPROTO_TCP )) == INVALID_SOCKET)
	{
		printf("Error al crear el socket : %d" , WSAGetLastError());
	}
	
	printf("Seleccione la ip destino: ");gets(ip);
	
	//Especificamos una Direccion para el Socket
	...
	if (connect(network_socket, (struct sockaddr *) &server_address, sizeof(server_address))!=-1){
		printf("Lo Lamento, Conexion rechazada\n");
	} 
	...
	//Quien Empieza??
	 ...
	 if(X==0){
		...
		send(network_socket,&nombre_jugador1,sizeof(nombre_jugador1),0);
		recv(network_socket, &nombre_jugador2, sizeof(nombre_jugador2),0);
	...
	send(network_socket, &fila, sizeof(fila),0);
	send(network_socket, &columna, sizeof(columna),0);
	...
	recv(network_socket,&fila2,sizeof(fila2),0);
	recv(network_socket,&columna2,sizeof(columna2),0);
	...
	//SI JUGADOR1 ES EL SERVIDOR
		...
		recv(network_socket, &nombre_jugador1, sizeof(nombre_jugador1),0);
		printf("\n El nombre del jugador 2 es: %s", nombre_jugador1);
		printf("\n Introduzca su nombre: ");gets(nombre_jugador2);
		send(network_socket, &nombre_jugador2, sizeof(nombre_jugador2),0);
	...
	recv(network_socket,&fila, sizeof(fila),0);
	recv(network_socket,&columna, sizeof(columna),0);
	...
	send(network_socket, &fila2, sizeof(fila2),0);
	send(network_socket, &columna2, sizeof(columna2),0);
